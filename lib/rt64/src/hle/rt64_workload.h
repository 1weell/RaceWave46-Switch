...
namespace RT64 {
    ...
    struct Workload {
        ...
        // Rebuilt once per native frame. A flat sorted map avoids allocating a
        // tree node for every transform while retaining ordered iteration.
        std::vector<std::pair<uint32_t, uint32_t>> transformIdMap;
