...
#define WR64_SWITCH_PRESENT_TRACE(message) wr64_switch_trace(message)
#else
#define WR64_SWITCH_PRESENT_TRACE(message) do { } while (0)
#endif
...
static constexpr int32_t Wr64PresentDrainFrames = 1;
    ...
    PresentQueue::~PresentQueue() {
        // Serialize shutdown with every local wait predicate to avoid a lost wakeup.
        {
            std::scoped_lock lock(cursorMutex, presentIdMutex);
    ...
    void PresentQueue::advanceToNextPresent() {
        {
            ...
            const int nextWriteCursor = (writeCursor + 1) % presents.size();
...
#ifdef __SWITCH__
                const Timestamp switchPerfPresentStart = Timer::current();
#endif
                swapChainValid = ext.swapChain->present(swapChainIndex, &waitSemaphore, 1);
#ifdef __SWITCH__
                const double switchPerfPresentMs = std::chrono::duration<double, std::milli>(Timer::current() - switchPerfPresentStart).count();
                const double switchPerfFrameMs = (previousPresentTimestamp != Timestamp())
                    ? std::chrono::duration<double, std::milli>(presentTimestamp - previousPresentTimestamp).count() : 0.0;
                wr64_switch_perf_present(switchPerfFrameMs, switchPerfPresentMs,
                    ext.swapChain->getWidth(), ext.swapChain->getHeight(),
                    viOriginalRate, targetRate, swapChainRate, swapChainValid);
#endif
                if (secondaryPresentFrame) {
    ...
    void PresentQueue::threadLoop() {
        ...
        WR64_SWITCH_PRESENT_TRACE(swapChainValid
            ? "[present] thread started with valid swapchain"
            : "[present] thread started; swapchain resize required");
        ...
        while (presentThreadRunning) {
            ...
            if (processCursor >= 0) {
                ...
                if (needsResize) {
                    ...
                    WR64_SWITCH_PRESENT_TRACE(swapChainValid
                        ? "[present] swapchain resize succeeded"
                        : "[present] swapchain resize failed");
