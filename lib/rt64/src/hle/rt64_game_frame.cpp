...
namespace RT64 {
    ...
    static bool wr64WaterSheetDumpEnabled() {
        if (!WR64_DIAGNOSTIC_ENABLED) {
            return false;
        }
        const char *value = std::getenv("WR64_WATER_SHEET_TRACE");
    ...
    static bool wr64TransformIdMapContains(
        const std::vector<std::pair<uint32_t, uint32_t>> &idMap, uint32_t matrixId);
            ...
            if (wr64GantryIdentityEnabled() && wr64GantryMatchTraceArmed()) {
                ...
                for (uint32_t t = 0; t < curWorkload.drawData.worldTransformGroups.size(); t++) {
                    ...
                    wr64GantryMatchRecord(wr64GantryId, wr64GantryClaimedCount,
                        wr64TransformIdMapContains(curWorkload.transformIdMap, wr64GantryId),
                        wr64TransformIdMapContains(prevWorkload.transformIdMap, wr64GantryId),
    ...
    void GameFrame::matchScene(WorkloadQueue &workloadQueue, const GameFrame &prevFrame, const GameScene &curScene, const GameScene &prevScene, std::unordered_map<uint32_t, ModifiedBuffers> &workloadsModified, bool &tileInterpolationUsed, bool &lookAtInterpolationUsed) {
        ...
        // These sets are rebuilt for every scene.  std::set makes every
        // insertion a separately allocated tree node, even though the values
        // are only consumed after collection and in sorted order.  Keep the
        // same ordering/uniqueness contract with flat vectors instead; this is
        // substantially cheaper on the Switch CPU and also improves locality
        // during the transform matching pass.
        thread_local std::vector<IndexPair> transformCheckSet;
        thread_local std::vector<IndexPair> tileCheckSet;
        thread_local std::vector<IndexPair> lookAtCheckSet;
        transformCheckSet.clear();
        tileCheckSet.clear();
        lookAtCheckSet.clear();
        thread_local std::vector<uint32_t> wr64WaterCurTransforms;
        wr64WaterCurTransforms.clear();
        thread_local std::vector<uint32_t> wr64ExactSkyCurTransforms;
        ...
        for (std::pair<uint64_t, GameCallMap> curIt : curCallHashMap) {
            ...
            if (wr64ExactSkyCall) {
                for (uint32_t m = waterCurCall.callDesc.minWorldMatrix;
                    m <= waterCurCall.callDesc.maxWorldMatrix; m++)
                {
                    wr64ExactSkyCurTransforms.push_back(m);
                }
            }
            ...
            if (wr64WaterCall) {
                for (uint32_t m = waterCurCall.callDesc.minWorldMatrix; m <= waterCurCall.callDesc.maxWorldMatrix; m++) {
                    wr64WaterCurTransforms.push_back(m);
                }
            ...
            for (auto prevIt = prevRange.first; prevIt != prevRange.second; prevIt++) {
                ...
                if ((curWorldMatrixCount == prevWorldMatrixCount) && curIt.second.doTransformMatching && prevIt->second.doTransformMatching) {
                    for (uint32_t w = 0; w < curWorldMatrixCount; w++) {
                        ...
                        if (matrixPairEligible) {
                            transformCheckSet.emplace_back(curWorldMatrix, prevWorldMatrix);
                        }
                    }
                }
                ...
                if ((curCall.callDesc.tileCount == prevCall.callDesc.tileCount) && (curIt.second.doTileInterpolation && prevIt->second.doTileInterpolation)) {
                    for (uint32_t t = 0; t < curCall.callDesc.tileCount; t++) {
                        ...
                        tileCheckSet.emplace_back(curCall.callDesc.tileIndex + t, prevCall.callDesc.tileIndex + t);
                    }
                }
                ...
                if (curUsesTextureGen && prevUsesTextureGen && (true && true)) { // TODO: Do look at matching condition.
                    ...
                    lookAtCheckSet.emplace_back(curLookAtIndex, prevLookAtIndex);
                }
            }
        }

        // Restore the exact ordering/uniqueness behavior of std::set after the
        // allocation-free collection phase.  The vectors retain their capacity
        // between native frames, so steady-state matching does not allocate.
        const auto sortUnique = [](auto &values) {
            std::sort(values.begin(), values.end());
            values.erase(std::unique(values.begin(), values.end()), values.end());
        };
        sortUnique(transformCheckSet);
        sortUnique(tileCheckSet);
        sortUnique(lookAtCheckSet);
        sortUnique(wr64WaterCurTransforms);
        sortUnique(wr64ExactSkyCurTransforms);
        ...
        for (const IndexPair &indices : transformCheckSet) {
            ...
            if (std::binary_search(wr64WaterCurTransforms.begin(), wr64WaterCurTransforms.end(), indices.first)) {
        ...
        std::stable_sort(matchCandidates.begin(), matchCandidates.end());
        for (const MatchCandidate candidate : matchCandidates) {
            const bool wr64WaterTransform = std::binary_search(
                wr64WaterCurTransforms.begin(), wr64WaterCurTransforms.end(), candidate.curIndex);
            const bool wr64ExactSkyTransform =
                std::binary_search(wr64ExactSkyCurTransforms.begin(),
                    wr64ExactSkyCurTransforms.end(), candidate.curIndex);
            ...
            if (addressRangesValid) {
                ...
                if (previousSamples.size() >= 4) {
                    ...
                    // guess.
                    {
                        ...
                        if (wr64WaterSheetDumpEnabled() && !sheetDone) {
    ...
    static bool wr64TransformIdMapContains(
        const std::vector<std::pair<uint32_t, uint32_t>> &idMap, uint32_t matrixId)
    {
        const auto it = std::lower_bound(idMap.begin(), idMap.end(), matrixId,
            [](const std::pair<uint32_t, uint32_t> &entry, uint32_t key) {
                return entry.first < key;
            });
        return it != idMap.end() && it->first == matrixId;
    }

    void GameFrame::buildTransformIdMap(const Workload &workload,
        std::vector<std::pair<uint32_t, uint32_t>> &idMap,
        std::vector<uint32_t> &ignoredIdVector) const
    {
        idMap.clear();
        ...
        // std::multimap iterates by key and preserves insertion order for
        // equivalent keys. stable_sort gives the same observable ordering
        // without per-entry tree allocations.
        std::stable_sort(idMap.begin(), idMap.end(),
