...
namespace RT64 {
    ...
    WorkloadQueue::~WorkloadQueue() {
        ...
        {
            std::scoped_lock lock(cursorMutex, idleMutex, workloadIdMutex);
    ...
    void WorkloadQueue::advanceToNextWorkload() {
        {
            ...
            const int nextWriteCursor = (writeCursor + 1) % workloads.size();
            // Keep the GPU-owned slot protected while yielding the producer's CPU.
...
#       endif
            ...
            wr64RendererGpuMs = double(frameTimestamps[1] - frameTimestamps[0]) / 1000000.0;
            rendererGPUProfiler.log(wr64RendererGpuMs);
        ...
        wr64_switch_perf_renderer(rendererCPUProfiler.accumulation, wr64RendererGpuMs);
#endif
...
#if WR64_ENABLE_DIAGNOSTICS && defined(WR64_SWITCH)
                    wr64_switch_perf_matching(matchingProfiler.accumulation);
#endif
...
#if WR64_ENABLE_DIAGNOSTICS && defined(WR64_SWITCH)
                wr64_switch_perf_workload(workloadProfiler.accumulation);
#endif
