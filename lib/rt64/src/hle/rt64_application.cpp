...
#ifdef __SWITCH__
        // VK_NN_vi_surface on Horizon exposes the RGBA8 format used by the
        // Mesa/NVK WSI. Requesting BGRA8 leaves Plume with no compatible
        // surface format and an inert swapchain (black screen, while the game
...
#ifdef __SWITCH__
        // Background specialization competes with game/render/audio threads;
        // the ubershader remains available while this single worker catches up.
        const uint32_t rasterShaderThreads = 1U;
#else
...
#   endif
    ...
    bool Application::loadConfiguration() {
        ...
        userConfig.validate();
#ifdef __SWITCH__
        // Migrate the first Switch config emitted by boot23. Its desktop
        // WindowIntegerScale default becomes 3x at 720p, which is too costly
        // for the handheld GPU. Only the untouched default tuple is changed;
        // explicit refresh-rate and resolution choices remain authoritative.
        const bool oldSwitchDefaults =
            readConfig &&
            userConfig.resolution == UserConfiguration::Resolution::WindowIntegerScale &&
            userConfig.resolutionMultiplier == 2.0 &&
            userConfig.downsampleMultiplier == 1 &&
            userConfig.filtering == UserConfiguration::Filtering::AntiAliasedPixelScaling &&
            userConfig.refreshRate == UserConfiguration::RefreshRate::Manual &&
            userConfig.refreshRateTarget == 60;
        if (oldSwitchDefaults) {
            userConfig.resolution = UserConfiguration::Resolution::Manual;
            userConfig.resolutionMultiplier = 1.0;
            userConfig.filtering = UserConfiguration::Filtering::Nearest;
            saveConfiguration();
            std::fprintf(stderr, "[rt64] migrated Switch defaults to native 1x resolution\n");
        }
#endif
        return readConfig;
    }
