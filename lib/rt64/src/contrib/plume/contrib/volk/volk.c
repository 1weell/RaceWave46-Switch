...
#define VOLK_SWITCH_NO_LOADER 1
#endif
...
#if defined(__SWITCH__)
/*
 * Mesa NVK is linked into Switch NROs as a loaderless static archive.  It
 * exposes the ICD bootstrap entrypoint instead of a shared libvulkan loader.
 * Keep this weak so builds without an NVK archive still fail cleanly at
 * runtime with VK_ERROR_INITIALIZATION_FAILED.
 */
...
#elif defined(__SWITCH__)
	// Switch homebrew has no shared Vulkan loader. When Mesa NVK is linked
	// into the NRO, use its loaderless ICD bootstrap directly.
	void* module = NULL;
	if (vk_icdGetInstanceProcAddr == NULL)
		return VK_ERROR_INITIALIZATION_FAILED;
