...
// Mesa's loaderless libvulkan exports real vk* entrypoints.  Volk normally
// defines global function-pointer variables with those same names, which
// collides with the static driver's symbols.  Give Volk's table a private
// prefix on Switch while keeping the public call sites unchanged.
#include "contrib/volk/volk_switch_aliases.h"
#endif
...
#   elif defined(__SWITCH__)
        ...
        surfaceCreateInfo.window = nwindowGetDefault();
        ...
        if (res != VK_SUCCESS) {
            ...
            wr64_switch_trace("[vulkan] VI surface creation failed");
            return;
        }
...
#   endif
        ...
        if (res != VK_SUCCESS) {
            ...
            wr64_switch_trace("[vulkan] surface support query failed");
            return;
        }

        if (!presentSupported) {
            fprintf(stderr, "Command queue does not support present.\n");
            wr64_switch_trace("[vulkan] command queue cannot present to VI surface");
            return;
        }
...
#if defined(__SWITCH__)
            wr64_switch_trace("[vulkan] surface capabilities query failed");
#endif
            return;
        }
#if defined(__SWITCH__)
        {
            ...
            std::snprintf(message, sizeof(message),
                          "[vulkan] caps extent=%ux%u min=%ux%u max=%ux%u images=%u..%u usage=0x%X transforms=0x%X current=0x%X alpha=0x%X",
                          ...
                          surfaceCapabilities.supportedUsageFlags, surfaceCapabilities.supportedTransforms,
                          surfaceCapabilities.currentTransform, surfaceCapabilities.supportedCompositeAlpha);
        ...
        {
            char message[192];
            const int first = surfaceFormatCount > 0 ? int(surfaceFormats[0].format) : -1;
            const int second = surfaceFormatCount > 1 ? int(surfaceFormats[1].format) : -1;
            std::snprintf(message, sizeof(message), "[vulkan] formats count=%u requested=%d first=%d second=%d",
                          surfaceFormatCount, int(toVk(desc.format)), first, second);
            wr64_switch_trace(message);
        }
#endif
        ...
        if (compatibleSurfaceFormats.empty()) {
            ...
            wr64_switch_trace("[vulkan] no compatible requested surface format");
            return;
        }
    ...
    bool VulkanSwapChain::present(uint32_t textureIndex, RenderCommandSemaphore **waitSemaphores, uint32_t waitSemaphoreCount) {
...
#endif
#if defined(__SWITCH__)
            wr64_switch_trace("[vulkan] queue present failed");
#endif
            return false;
        }
        ...
        if (firstPresent) {
            wr64_switch_trace("[vulkan] first frame presented");
            firstPresent = false;
        }
#endif
        return true;
    ...
    bool VulkanSwapChain::resize() {
        ...
        if ((width == 0) || (height == 0)) {
#if defined(__SWITCH__)
            wr64_switch_trace("[vulkan] resize rejected zero window dimensions");
#endif
            return false;
        }
...
#if defined(__SWITCH__)
        // NVK's VI swapchain path uses a transfer into the scanout image after
        // rendering. Match the proven Switch WSI sample while masking the bit
        // against the capabilities reported by this surface.
        ...
        // Horizon ports report the transform they actually support through the
        // surface capabilities.  Identity is preferred when available, but
        // currentTransform is the portable fallback used by other Switch ports.
        createInfo.preTransform = (surfaceCapabilities.supportedTransforms & VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR) != 0
...
#if defined(__SWITCH__)
            char message[128];
            std::snprintf(message, sizeof(message), "[vulkan] resize create swapchain failed %d dims=%ux%u usage=0x%X format=%d", int(res), width, height, createInfo.imageUsage, int(createInfo.imageFormat));
            wr64_switch_trace(message);
#endif
            return false;
        }
        ...
        {
            char message[96];
            std::snprintf(message, sizeof(message), "[vulkan] resize created swapchain dims=%ux%u", width, height);
            wr64_switch_trace(message);
        }
#endif
...
#   elif defined(__SWITCH__)
        // The surface is created from Horizon's default NWindow, so its
        // dimensions are authoritative. SDL_GetWindowSizeInPixels can report
        // zero for the bookkeeping-only SDL window on the Switch port, which
        // prevents resize() from ever creating a Vulkan swapchain.
        dstWidth = 0;
        dstHeight = 0;
        const Result rc = nwindowGetDimensions(nwindowGetDefault(), &dstWidth, &dstHeight);
        if (R_FAILED(rc) || (dstWidth == 0) || (dstHeight == 0)) {
            dstWidth = 1280;
            dstHeight = 720;
            wr64_switch_trace("[vulkan] native window dimensions unavailable; using 1280x720");
        }
#   elif defined(PLUME_SDL_VULKAN_ENABLED)
...
#   endif
    ...
    bool VulkanSwapChain::acquireTexture(RenderCommandSemaphore *signalSemaphore, uint32_t *textureIndex) {
        ...
        if ((res != VK_SUCCESS) && (res != VK_SUBOPTIMAL_KHR)) {
#if defined(__SWITCH__)
            wr64_switch_trace("[vulkan] acquire swapchain image failed");
#endif
            return false;
        }
        ...
        if (firstAcquire) {
            wr64_switch_trace("[vulkan] first swapchain image acquired");
            firstAcquire = false;
        }
#endif
        return true;
...
#       endif
        ...
        if (!disableInternalCacheRequested) {
            // flags = 0 keeps cache access internally synchronized by Vulkan, allowing
            // concurrent pipeline workers without serializing compilation in the application.
            // Keep this cache in memory only; no merge or serialization runs alongside workers.
            VkPipelineCacheCreateInfo cacheInfo = {};
            cacheInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO;
            VkPipelineCache newCache = VK_NULL_HANDLE;
            const VkResult cacheResult = vkCreatePipelineCache(vk, &cacheInfo, nullptr, &newCache);
            if (cacheResult == VK_SUCCESS) {
                pipelineCache = newCache;
            }
            else {
                // Caching is optional. Preserve the uncached pipeline creation path on failure.
                fprintf(stderr, "vkCreatePipelineCache failed with error code 0x%X; continuing without pipeline cache.\n", cacheResult);
            }
        }
...
#   endif
    ...
    void VulkanDevice::release() {
        ...
        if (vk != VK_NULL_HANDLE) {
            // As with device destruction, callers must have stopped all pipeline workers.
...
#if PLUME_SDL_VULKAN_ENABLED || defined(__SWITCH__)
