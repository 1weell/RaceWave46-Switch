...
void vi_thread_func() {
    WR64_SWITCH_TRACE("[vi-thread] entered");
...
void task_thread_func(uint8_t* rdram, moodycamel::LightweightSemaphore* thread_ready) {
    WR64_SWITCH_TRACE("[task-thread] entered");
...
#ifdef __SWITCH__
static void trace_renderer_setup_result(ultramodern::renderer::SetupResult result) {
    switch (result) {
        case ultramodern::renderer::SetupResult::Success:
            WR64_SWITCH_TRACE("[renderer] setup result: Success");
            break;
        case ultramodern::renderer::SetupResult::DynamicLibrariesNotFound:
            WR64_SWITCH_TRACE("[renderer] setup result: DynamicLibrariesNotFound");
            break;
        case ultramodern::renderer::SetupResult::InvalidGraphicsAPI:
            WR64_SWITCH_TRACE("[renderer] setup result: InvalidGraphicsAPI");
            break;
        case ultramodern::renderer::SetupResult::GraphicsAPINotFound:
            WR64_SWITCH_TRACE("[renderer] setup result: GraphicsAPINotFound");
            break;
        case ultramodern::renderer::SetupResult::GraphicsDeviceNotFound:
            WR64_SWITCH_TRACE("[renderer] setup result: GraphicsDeviceNotFound");
            break;
    }
}
#else
static void trace_renderer_setup_result(ultramodern::renderer::SetupResult) {
}
#endif
...
void gfx_thread_func(uint8_t* rdram, moodycamel::LightweightSemaphore* thread_ready, ultramodern::renderer::WindowHandle window_handle) {
    WR64_SWITCH_TRACE("[gfx-thread] entered");
    ...
    WR64_SWITCH_TRACE("[gfx-thread] creating renderer context");
    ...
    WR64_SWITCH_TRACE("[gfx-thread] renderer context created");

    renderer_chosen_api.store(renderer_context->get_chosen_api());
    if (!renderer_context->valid()) {
        const auto setup_result = renderer_context->get_setup_result();
        renderer_setup_result.store(setup_result);
        trace_renderer_setup_result(setup_result);
        // Notify the caller thread that this thread is ready.
        thread_ready->signal();
        return;
    }
    ...
    WR64_SWITCH_TRACE("[gfx-thread] RSP initialized; signaling ready");
    ...
    while (!exited) {
        ...
        if (events_context.action_queue.wait_dequeue_timed(action, 1ms)) {
            ...
            if (const auto* task_action = std::get_if<SpTaskAction>(&action)) {
                ...
                if (first_task) WR64_SWITCH_TRACE("[gfx-thread] first display list received");
                ...
                if (first_task) {
                    WR64_SWITCH_TRACE("[gfx-thread] first display list processed");
                    first_task = false;
                }
                ...
                if (first_update) WR64_SWITCH_TRACE("[gfx-thread] first screen update entering");
                events_context.vi.update_screen_regs = screen_update_action->regs;
                renderer_context->update_screen();
                if (first_update) {
                    WR64_SWITCH_TRACE("[gfx-thread] first screen update returned");
                    first_update = false;
                }
                ...
                static bool first_dummy = true;
                if (first_dummy) WR64_SWITCH_TRACE("[gfx-thread] first dummy workload entering");
                renderer_context->send_dummy_workload(dummy_workload_action->fb_address);
                if (first_dummy) {
                    WR64_SWITCH_TRACE("[gfx-thread] first dummy workload returned");
                    first_dummy = false;
                }
            }
        }
...
extern "C" void osViSwapBuffer(RDRAM_ARG PTR(void) frameBufPtr) {
    std::lock_guard lock{ events_context.message_mutex };
#ifdef __SWITCH__
    static unsigned int logged_swaps = 0;
    if (logged_swaps++ < 4) {
        char message[96];
        std::snprintf(message, sizeof(message), "[vi] swap framebuffer=0x%08X", static_cast<unsigned int>(frameBufPtr));
...
extern "C" void osViSetMode(RDRAM_ARG PTR(OSViMode) mode_) {
    WR64_SWITCH_TRACE("[vi] game set VI mode");
    std::lock_guard lock{ events_context.message_mutex };
...
extern "C" void osViBlack(uint8_t active) {
    WR64_SWITCH_TRACE(active ? "[vi] black enabled" : "[vi] black disabled");
    std::lock_guard lock{ events_context.message_mutex };
    ViState* next_state = events_context.vi.get_next_state();
...
void ultramodern::init_events(RDRAM_ARG ultramodern::renderer::WindowHandle window_handle) {
    WR64_SWITCH_TRACE("[events] creating graphics thread");
    ...
    WR64_SWITCH_TRACE("[events] creating task thread");
    ...
    WR64_SWITCH_TRACE("[events] waiting for graphics thread");
    gfx_thread_ready.wait();
    WR64_SWITCH_TRACE("[events] graphics thread ready; waiting for task thread");
    task_thread_ready.wait();
    WR64_SWITCH_TRACE("[events] task thread ready");
    ...
    WR64_SWITCH_TRACE("[events] creating VI thread");
    events_context.vi.thread = std::thread{ vi_thread_func };
    WR64_SWITCH_TRACE("[events] VI thread created");
}
