...
static void trace_n64_thread(const char* phase, const OSThread* t) {
    char message[128];
    std::snprintf(message, sizeof(message), "[n64-thread] %s id=%d priority=%d", phase, t->id, t->priority);
    wr64_switch_trace(message);
}
#else
static void trace_n64_thread(const char*, const OSThread*) {}
#endif
...
// libnx does not expose a portable native priority/name API.  Keeping these
// hooks as no-ops preserves the runtime scheduler without introducing a
// platform-specific dependency into the recompiler core.
void ultramodern::set_native_thread_name(const std::string&) {}
void ultramodern::set_native_thread_priority(ThreadPriority) {}
#endif
...
extern "C" void osStartThread(RDRAM_ARG PTR(OSThread) t_) {
    ...
    trace_n64_thread("start requested", t);
