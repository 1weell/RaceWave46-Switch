...
void timer_thread(RDRAM_ARG1) {
    ...
    auto process_timer_action = [&](const Action& action) {
        if (std::holds_alternative<StopTimerAction>(action)) {
            return false;
        }
    ...
    while (true) {
        ...
        while (timer_context.action_queue.try_dequeue(cur_action)) {
            if (!process_timer_action(cur_action)) return;
        }
...
void ultramodern::shutdown_timers() {
