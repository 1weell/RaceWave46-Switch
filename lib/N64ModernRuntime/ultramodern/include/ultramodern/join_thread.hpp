...
inline void join_thread_servicing_window_messages(std::thread& thread) {
    // Some runtime paths can fail before all event threads are created (for
    // example when the renderer cannot initialize).  Joining a default
    // constructed std::thread throws EINVAL and masks the original error.
    if (!thread.joinable()) {
        return;
    }
#ifdef _WIN32
