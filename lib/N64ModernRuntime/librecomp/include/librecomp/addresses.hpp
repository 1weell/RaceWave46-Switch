...
namespace recomp {
    // The desktop runtime reserves the full 512 MiB kseg0 window. Switch
    // homebrew has a much smaller committed heap; Wave Race uses less than
    // 16 MiB (including the recomp/mod heap), so retain ample headroom without
    // consuming most of the process memory before graphics starts.
#ifdef __SWITCH__
    constexpr size_t mem_size = 128ULL * 1024ULL * 1024ULL;
#else
