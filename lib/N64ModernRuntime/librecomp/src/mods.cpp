...
// Dynamic mod loading and live patch protection are host features. Switch
// builds use the statically linked recompilation and keep this API inert.
...
public:
    ...
    template <typename T>
    bool get_dll_symbol(T&, const char*) const { return false; }
    uint32_t get_api_version() { return 0; }
};

void unprotect(void*, uint64_t*) {}
...
// The desktop live recompiler patches executable memory and depends on the
// host JIT/sljit backend. Switch builds keep the mod ABI present but report
// hooks as unavailable instead of pulling that host-only implementation into
// the final NRO.
...
recomp::mods::CodeModLoadError recomp::mods::ModContext::load_mod_code(uint8_t* rdram, recomp::mods::ModHandle& mod, uint32_t base_event_index, std::string& error_param) {
    ...
    (void)handle_inputs;
    error_param = "live recompiler is unavailable on Nintendo Switch";
    return CodeModLoadError::HooksUnavailable;
#else
    else {
