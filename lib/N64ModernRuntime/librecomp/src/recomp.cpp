...
#ifdef __SWITCH__
    // The headless Switch bootstrap selects the ROM immediately before the
    // runtime starts. Keep that validated image in memory: fsdev/FAT rename
    // semantics are not needed here and previously turned a valid ROM into a
    // generic launch failure while trying to create a redundant cache copy.
...
recomp::RomValidationError recomp::select_rom(const std::filesystem::path& rom_path, std::u8string& game_id) {
    ...
    valid_game_roms.insert(game_id);
#else
...
bool wait_for_game_started(uint8_t* rdram, recomp_context* context) {
    ...
    switch (game_status.load()) {
        ...
        case GameStatus::Running:
            {
                WR64_SWITCH_TRACE("[game-start] loading stored ROM");
                ...
                try {
                    WR64_SWITCH_TRACE("[game-start] entering recompiled entrypoint");
                    game_entry.entrypoint(rdram, context);
                    WR64_SWITCH_TRACE("[game-start] entrypoint returned; N64 workers continue");
        ...
        case GameStatus::Quit:
            WR64_SWITCH_TRACE("[game-start] quit requested before launch");
            return true;
    ...
    // Keep malformed/future status values from falling through a non-void
    // function. Treat them as a stop request instead of invoking UB.
    return true;
}
...
void recomp::start(const recomp::Configuration& cfg) {
    WR64_SWITCH_TRACE("[start] entered");
    project_version = cfg.project_version;
    WR64_SWITCH_TRACE("[start] checking stored ROMs");
    recomp::check_all_stored_roms();
    WR64_SWITCH_TRACE("[start] stored ROM check complete");

    recomp::rsp::set_callbacks(cfg.rsp_callbacks);
    WR64_SWITCH_TRACE("[start] RSP callbacks installed");
    ...
    WR64_SWITCH_TRACE("[start] runtime callbacks installed");
    ...
    WR64_SWITCH_TRACE("[start] graphics host created");
...
#ifdef __SWITCH__
    if (window_handle == ultramodern::renderer::WindowHandle{}) {
        WR64_SWITCH_TRACE("[start] window handle is null");
    }
    else {
        WR64_SWITCH_TRACE("[start] window handle ready");
    }
#else
    WR64_SWITCH_TRACE("[start] window handle ready");
#endif
    ...
    WR64_SWITCH_TRACE("[start] message queue control installed");

    WR64_SWITCH_TRACE("[start] initializing mods");
    recomp::mods::initialize_mods();
    WR64_SWITCH_TRACE("[start] scanning mods");
    recomp::mods::scan_mods();
    WR64_SWITCH_TRACE("[start] mods ready");
...
#elif defined(__SWITCH__)
    // Desktop reserves a 4 GiB virtual range but commits only mem_size.
    // calloc commits real Switch heap memory, so allocate that same usable
    // region rather than exhausting memory with the virtual guard range.
    rdram = reinterpret_cast<uint8_t*>(calloc(1, mem_size));
    alloc_failed = (rdram == nullptr);
#else
    ...
    if (alloc_failed) {
        WR64_SWITCH_TRACE("[start] RDRAM allocation failed");
        ultramodern::error_handling::message_box("Failed to allocate memory!");
        return;
    }
    WR64_SWITCH_TRACE("[start] RDRAM allocated");

    recomp::register_heap_exports();
    recomp::mods::register_config_exports();
    recomp::mods::register_hook_exports();
    WR64_SWITCH_TRACE("[start] runtime exports registered");

    std::thread game_thread{[](ultramodern::renderer::WindowHandle window_handle, uint8_t* rdram) {
        WR64_SWITCH_TRACE("[game-thread] entered");
        debug_printf("[Recomp] Starting\n");

        ultramodern::set_native_thread_name("Game Start Thread");

        // Renderer setup runs on this worker. On platforms where the
        // selected graphics backend is unavailable, preinit reports a normal
        // setup failure by throwing; letting that exception escape a thread
        // invokes std::terminate and gives Switch only its generic crash
        // dialog. Convert it into a logged, orderly shutdown instead.
        try {
            WR64_SWITCH_TRACE("[game-thread] entering preinit");
            ultramodern::preinit(rdram, window_handle);
            WR64_SWITCH_TRACE("[game-thread] preinit complete");
        }
        catch (const std::exception& error) {
            WR64_SWITCH_TRACE("[game-thread] preinit exception");
            std::fprintf(stderr, "[runtime] preinit failed: %s\n", error.what());
            ultramodern::error_handling::message_box(error.what());
            ultramodern::quit();
            return;
        }
        catch (...) {
            WR64_SWITCH_TRACE("[game-thread] preinit unknown exception");
            std::fprintf(stderr, "[runtime] preinit failed with an unknown exception\n");
            ultramodern::error_handling::message_box("Renderer preinit failed with an unknown exception");
            ultramodern::quit();
            return;
        }

        recomp_context context{};
        WR64_SWITCH_TRACE("[game-thread] waiting for game start");

        // Loop until the game starts.
        while (!wait_for_game_started(rdram, &context)) {}
        WR64_SWITCH_TRACE("[game-thread] startup worker finished");
    }, window_handle, rdram};
    WR64_SWITCH_TRACE("[start] game worker created");

    while (!exited) {
#ifdef __SWITCH__
        // Honor HOME/close requests through libnx's normal applet lifecycle.
        // The previous timed diagnostic called quit after ten seconds, then a
        // forwarder could turn the resulting process exit into 2168-0002.
        if (!wr64_switch_applet_main_loop()) {
            WR64_SWITCH_TRACE("[start] Horizon requested shutdown");
            ultramodern::quit();
            break;
        }
#endif
    ...
    // timer.cpp used to detach this worker. libnx rejects pthread_detach with
    // EINVAL, and a detached timer could also outlive and access freed RDRAM.
    // Stop and join it as part of the normal runtime lifecycle instead.
    ultramodern::shutdown_timers();
