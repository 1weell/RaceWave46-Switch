...
void update_save_file() {
...
#ifdef __SWITCH__
    // libstdc++ filesystem copy_file is unreliable on fsdev. Write the tiny
    // EEPROM image directly; otherwise every in-game EEPROM update reports a
    // failure even though the containing SD directory is writable.
    {
        std::ofstream save_file{ultramodern::get_save_file_path(),
    ...
    if (saving_failed) {
        ...
        if (first_save) {
            wr64_switch_trace("[save] first EEPROM write succeeded");
            first_save = false;
        }
    }
#endif
