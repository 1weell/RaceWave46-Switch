...
#else
#ifdef WR64_SWITCH
    // Homebrew launches from an implementation-defined working directory;
    // keep the portable profile on the SD card unless the user overrides it.
    return parse_options(argc, argv, std::filesystem::u8path("sdmc:/switch/RaceWave46/.runtime"));
#else
...
Options parse_options(int argc, char** argv, const std::filesystem::path& default_runtime) {
    Options result;
#ifdef WR64_SWITCH
    result.runtime = default_runtime.empty()
        ? std::filesystem::u8path("sdmc:/switch/RaceWave46/.runtime")
        : default_runtime;
#else
...
#ifdef WR64_SWITCH
            result.runtime = std::filesystem::u8path(argument(i));
#else
            result.runtime = std::filesystem::absolute(std::filesystem::u8path(argument(i)));
#endif
        }
