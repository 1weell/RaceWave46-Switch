...
#if WR64_ENABLE_DIAGNOSTICS && WR64_ENABLE_FORENSIC
...
constexpr size_t DefaultMegabytes = 16;
#else
...
extern "C" void wr64_forensic_initialize(void) {
    ...
    try {
        ...
        gRecorder.capacity = 0;
        gRecorder.initialized.store(false, std::memory_order_release);
        std::fprintf(stderr, "[forensic] recorder disabled: allocation failed\n");
        return;
    }
    std::fprintf(stderr,
