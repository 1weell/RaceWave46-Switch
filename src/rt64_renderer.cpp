...
#include "wr64_switch_stage_log.h"
#include "common/rt64_wr64_perf.h"
...
#ifdef WR64_SWITCH
// newlib provides setenv on Switch, but its C++ stdlib wrapper does not
// expose the declaration with this toolchain configuration.
...
struct Wr64SwitchPerfState {
    std::atomic<uint64_t> viUpdates{0};
    std::atomic<uint64_t> presentAttempts{0};
    std::atomic<uint64_t> presented{0};
    std::atomic<uint64_t> presentFailures{0};
    std::atomic<uint64_t> frameUs{0};
    std::atomic<uint64_t> presentUs{0};
    std::atomic<uint64_t> displayListUs{0};
    std::atomic<uint64_t> displayListCount{0};
    std::atomic<uint64_t> screenUpdateUs{0};
    std::atomic<uint64_t> screenUpdateCount{0};
    std::atomic<uint64_t> matchingUs{0};
    std::atomic<uint64_t> matchingCount{0};
    std::atomic<uint64_t> workloadUs{0};
    std::atomic<uint64_t> workloadCount{0};
    std::atomic<uint64_t> rendererCpuUs{0};
    std::atomic<uint64_t> rendererGpuUs{0};
    std::atomic<uint64_t> rendererCount{0};
    std::atomic<uint32_t> width{0};
    std::atomic<uint32_t> height{0};
    std::atomic<uint32_t> nativeRate{0};
    std::atomic<uint32_t> targetRate{0};
    std::atomic<uint32_t> swapChainRate{0};
    std::atomic<uint32_t> presentedFpsMilli{0};
    std::atomic<uint32_t> viFpsMilli{0};
    std::atomic<uint32_t> frameMsMilli{0};
    std::atomic<uint32_t> presentMsMilli{0};
    std::atomic<uint32_t> snapshotPresented{0};
    std::atomic<uint32_t> snapshotViUpdates{0};
    std::atomic<uint32_t> snapshotAttempts{0};
    std::atomic<uint32_t> snapshotFailures{0};
    std::atomic<bool> presentReady{false};
    std::mutex logMutex;
    std::atomic<int64_t> windowStartUs{0};
};

Wr64SwitchPerfState wr64_switch_perf_state;

extern "C" void wr64_switch_trace(const char *message);

void wr64_switch_perf_publish() {
#ifdef WR64_SWITCH
    auto &state = wr64_switch_perf_state;
    const auto now = std::chrono::steady_clock::now();
    const int64_t nowUs = std::chrono::duration_cast<std::chrono::microseconds>(now.time_since_epoch()).count();
    int64_t windowStartUs = state.windowStartUs.load(std::memory_order_acquire);
    if (windowStartUs == 0 && state.windowStartUs.compare_exchange_strong(
            windowStartUs, nowUs, std::memory_order_acq_rel)) {
        return;
    }
    if (windowStartUs == 0) {
        windowStartUs = state.windowStartUs.load(std::memory_order_acquire);
    }
    if ((nowUs - windowStartUs) < 1'000'000) {
        return;
    }

    // Only the once-per-second rollover takes the mutex. Every frame before
    // the rollover is two relaxed atomics and a timestamp comparison.
    std::scoped_lock lock(state.logMutex);
    windowStartUs = state.windowStartUs.load(std::memory_order_acquire);
    if ((nowUs - windowStartUs) < 1'000'000) {
        return;
    }
    const double elapsed = static_cast<double>(nowUs - windowStartUs) / 1'000'000.0;

    const uint64_t viUpdates = state.viUpdates.exchange(0, std::memory_order_relaxed);
    const uint64_t attempts = state.presentAttempts.exchange(0, std::memory_order_relaxed);
    const uint64_t presented = state.presented.exchange(0, std::memory_order_relaxed);
    const uint64_t failures = state.presentFailures.exchange(0, std::memory_order_relaxed);
    const uint64_t frameUs = state.frameUs.exchange(0, std::memory_order_relaxed);
    const uint64_t presentUs = state.presentUs.exchange(0, std::memory_order_relaxed);
    const uint64_t displayListUs = state.displayListUs.exchange(0, std::memory_order_relaxed);
    const uint64_t displayListCount = state.displayListCount.exchange(0, std::memory_order_relaxed);
    const uint64_t screenUpdateUs = state.screenUpdateUs.exchange(0, std::memory_order_relaxed);
    const uint64_t screenUpdateCount = state.screenUpdateCount.exchange(0, std::memory_order_relaxed);
    const uint64_t matchingUs = state.matchingUs.exchange(0, std::memory_order_relaxed);
    const uint64_t matchingCount = state.matchingCount.exchange(0, std::memory_order_relaxed);
    const uint64_t workloadUs = state.workloadUs.exchange(0, std::memory_order_relaxed);
    const uint64_t workloadCount = state.workloadCount.exchange(0, std::memory_order_relaxed);
    const uint64_t rendererCpuUs = state.rendererCpuUs.exchange(0, std::memory_order_relaxed);
    const uint64_t rendererGpuUs = state.rendererGpuUs.exchange(0, std::memory_order_relaxed);
    const uint64_t rendererCount = state.rendererCount.exchange(0, std::memory_order_relaxed);
    const double presentedFps = static_cast<double>(presented) / elapsed;
    const double viFps = static_cast<double>(viUpdates) / elapsed;
    const double frameMs = presented > 0 ? static_cast<double>(frameUs) / (1000.0 * presented) : 0.0;
    const double presentMs = attempts > 0 ? static_cast<double>(presentUs) / (1000.0 * attempts) : 0.0;

    state.presentedFpsMilli.store(static_cast<uint32_t>(presentedFps * 1000.0), std::memory_order_relaxed);
    state.viFpsMilli.store(static_cast<uint32_t>(viFps * 1000.0), std::memory_order_relaxed);
    state.frameMsMilli.store(static_cast<uint32_t>(frameMs * 1000.0), std::memory_order_relaxed);
    state.presentMsMilli.store(static_cast<uint32_t>(presentMs * 1000.0), std::memory_order_relaxed);
    state.snapshotPresented.store(static_cast<uint32_t>(presented), std::memory_order_relaxed);
    state.snapshotViUpdates.store(static_cast<uint32_t>(viUpdates), std::memory_order_relaxed);
    state.snapshotAttempts.store(static_cast<uint32_t>(attempts), std::memory_order_relaxed);
    state.snapshotFailures.store(static_cast<uint32_t>(failures), std::memory_order_relaxed);
    state.windowStartUs.store(nowUs, std::memory_order_release);

    // Keep the on-screen snapshot in release builds, but do not perform
    // synchronous SD-card logging from VI/present threads every second.
#if WR64_ENABLE_DIAGNOSTICS
    const auto averageMs = [](uint64_t totalUs, uint64_t count) {
        return count > 0 ? static_cast<double>(totalUs) / (1000.0 * static_cast<double>(count)) : 0.0;
    };
    char message[512];
    std::snprintf(message, sizeof(message),
        "[perf] fps=%.1f vi=%.1f frame_ms=%.2f present_ms=%.2f dl_ms=%.2f dl_n=%llu "
        "screen_ms=%.2f screen_n=%llu match_ms=%.2f match_n=%llu workload_ms=%.2f workload_n=%llu "
        "render_cpu_ms=%.2f render_gpu_ms=%.2f render_n=%llu presented=%u attempts=%u failures=%u "
        "%ux%u native=%u target=%u display=%u",
        presentedFps, viFps, frameMs, presentMs,
        averageMs(displayListUs, displayListCount), static_cast<unsigned long long>(displayListCount),
        averageMs(screenUpdateUs, screenUpdateCount), static_cast<unsigned long long>(screenUpdateCount),
        averageMs(matchingUs, matchingCount), static_cast<unsigned long long>(matchingCount),
        averageMs(workloadUs, workloadCount), static_cast<unsigned long long>(workloadCount),
        averageMs(rendererCpuUs, rendererCount), averageMs(rendererGpuUs, rendererCount),
        static_cast<unsigned long long>(rendererCount),
        static_cast<unsigned>(presented), static_cast<unsigned>(attempts),
        static_cast<unsigned>(failures), state.width.load(std::memory_order_relaxed),
        state.height.load(std::memory_order_relaxed),
        state.nativeRate.load(std::memory_order_relaxed),
        state.targetRate.load(std::memory_order_relaxed),
        state.swapChainRate.load(std::memory_order_relaxed));
    wr64_switch_trace(message);
#endif
#endif
}
...
class RT64Context;
RT64Context *active_context = nullptr;
...
public:
...
#ifdef WR64_SWITCH
        // A failed SDL window creation used to fall through into
        // ApplicationWindow::setup(nullptr), which eventually dereferenced a
        // null SDL_Window on the console and produced only Horizon's generic
        // crash dialog. Treat it as a normal renderer setup failure instead.
        if (window_handle == ultramodern::renderer::WindowHandle{}) {
            std::fprintf(stderr, "[rt64] Switch window creation returned null\n");
...
#ifdef WR64_SWITCH
        // The desktop frontend is intentionally not part of the initial Switch
        // port. Let RT64 persist its native graphics settings in the runtime
        // directory so users can edit resolution, refresh rate, filtering and
        // downsampling without a desktop UI.
        config.useConfigurationFile = true;
#else
        // Frontend graphics.json is authoritative on desktop. Do not load a
        // competing RT64 configuration file over the selected edition's UI.
        config.useConfigurationFile = false;
#endif
        wr64_switch_stage_log("before RT64::Application construction");
        WR64_DIAGNOSTIC_LOG(std::fprintf(stderr, "[rt64] creating application\n"));
        app = std::make_unique<RT64::Application>(core, config);
        wr64_switch_stage_log("after RT64::Application construction");
#ifndef WR64_SWITCH
...
#endif
#ifdef WR64_SWITCH
        // Once Vulkan owns the NWindow, libnx's console-backed stderr can
        // point at a null software-console framebuffer. RT64/Mesa may emit a
        // diagnostic during setup, so keep stderr on SD instead of letting
        // ConsoleSwRenderer_drawChar write through the dead console surface.
        if (std::freopen("sdmc:/switch/RaceWave46/rt64-stderr.log", "ab", stderr) != nullptr) {
            std::setvbuf(stderr, nullptr, _IONBF, 0);
        }
#if defined(WR64_ENABLE_DIAGNOSTICS)
        // A stale Mesa/NVK disk shader cache can feed invalid IR into the
        // compiler before the first frame. Keep the diagnostic build clean so
        // we can distinguish cache corruption from a renderer ABI failure.
        setenv("MESA_SHADER_CACHE_DISABLE", "true", 1);
        wr64_switch_stage_log("Mesa shader cache disabled");
#endif
        wr64_switch_stage_log("before RT64::Application::setup");
        setup_result = map_setup_result(app->setup(0));
        wr64_switch_stage_log("after RT64::Application::setup");
#else
    ...
    ~RT64Context() override {
        if (active_context == this) active_context = nullptr;
    }
    ...
    void send_dl(const OSTask* task) override {
        if (app == nullptr || app->state == nullptr || app->interpreter == nullptr) {
            return;
        }
#if WR64_ENABLE_DIAGNOSTICS && defined(WR64_SWITCH)
...
#if WR64_ENABLE_DIAGNOSTICS && defined(WR64_SWITCH)
        wr64_switch_perf_display_list(std::chrono::duration<double, std::milli>(
...
#endif
#if WR64_ENABLE_DIAGNOSTICS && defined(WR64_SWITCH)
        wr64_switch_perf_screen_update(std::chrono::duration<double, std::milli>(
            std::chrono::steady_clock::now() - perfStart).count());
#endif
    }
...
private:
    ...
    void report_graphics_settings() const {
        ...
        const char *resolutionMode = "original";
        ...
        char message[256];
        std::snprintf(message, sizeof(message),
            "[rt64] applied graphics resolution=%s scale=%.2f downsample=%d refresh=%s target=%d",
            resolutionMode, static_cast<double>(get_resolution_scale()),
            config.downsampleMultiplier, refresh_rate_name(config.refreshRate), config.refreshRateTarget);
        wr64_switch_trace(message);
#endif
    }
...
public:
    RT64::Application *application() const { return app.get(); }
    void report_graphics_settings_public() const { report_graphics_settings(); }
...
#ifdef WR64_SWITCH
    wr64_switch_perf_state.viUpdates.fetch_add(1, std::memory_order_relaxed);
    wr64_switch_perf_publish();
#endif
}
...
#ifdef WR64_SWITCH
    auto &state = wr64_switch_perf_state;
    state.presentAttempts.fetch_add(1, std::memory_order_relaxed);
    if (success) {
        state.presentReady.store(true, std::memory_order_release);
        state.presented.fetch_add(1, std::memory_order_relaxed);
        state.frameUs.fetch_add(static_cast<uint64_t>(frameMs * 1000.0), std::memory_order_relaxed);
    }
    else {
        state.presentFailures.fetch_add(1, std::memory_order_relaxed);
    }
    state.presentUs.fetch_add(static_cast<uint64_t>(presentMs * 1000.0), std::memory_order_relaxed);
    state.width.store(width, std::memory_order_relaxed);
    state.height.store(height, std::memory_order_relaxed);
    state.nativeRate.store(nativeRate, std::memory_order_relaxed);
    state.targetRate.store(targetRate, std::memory_order_relaxed);
    state.swapChainRate.store(swapChainRate, std::memory_order_relaxed);
    wr64_switch_perf_publish();
#else
    (void)frameMs;
    (void)presentMs;
...
namespace {
void wr64_switch_perf_add_duration(std::atomic<uint64_t> &totalUs,
    std::atomic<uint64_t> &count, double durationMs) {
#ifdef WR64_SWITCH
    if (durationMs < 0.0) return;
    totalUs.fetch_add(static_cast<uint64_t>(durationMs * 1000.0), std::memory_order_relaxed);
    count.fetch_add(1, std::memory_order_relaxed);
#else
    (void)totalUs; (void)count; (void)durationMs;
#endif
}
}

extern "C" void wr64_switch_perf_display_list(double cpuMs) {
    wr64_switch_perf_add_duration(wr64_switch_perf_state.displayListUs,
        wr64_switch_perf_state.displayListCount, cpuMs);
}

extern "C" void wr64_switch_perf_screen_update(double cpuMs) {
    wr64_switch_perf_add_duration(wr64_switch_perf_state.screenUpdateUs,
        wr64_switch_perf_state.screenUpdateCount, cpuMs);
}

extern "C" void wr64_switch_perf_matching(double cpuMs) {
    wr64_switch_perf_add_duration(wr64_switch_perf_state.matchingUs,
        wr64_switch_perf_state.matchingCount, cpuMs);
}

extern "C" void wr64_switch_perf_workload(double cpuMs) {
    wr64_switch_perf_add_duration(wr64_switch_perf_state.workloadUs,
        wr64_switch_perf_state.workloadCount, cpuMs);
}

extern "C" void wr64_switch_perf_renderer(double cpuMs, double gpuMs) {
#ifdef WR64_SWITCH
    if (cpuMs < 0.0 || gpuMs < 0.0) return;
    auto &state = wr64_switch_perf_state;
    state.rendererCpuUs.fetch_add(static_cast<uint64_t>(cpuMs * 1000.0), std::memory_order_relaxed);
    state.rendererGpuUs.fetch_add(static_cast<uint64_t>(gpuMs * 1000.0), std::memory_order_relaxed);
    state.rendererCount.fetch_add(1, std::memory_order_relaxed);
#else
    (void)cpuMs; (void)gpuMs;
#endif
}

void switch_menu_get_settings(int *scale, int *refresh, int *filtering, int *downsample) {
#ifdef WR64_SWITCH
    if (active_context == nullptr || active_context->application() == nullptr) return;
    const auto &config = active_context->application()->userConfig;
    if (scale != nullptr) *scale = std::clamp(static_cast<int>(std::lround(config.resolutionMultiplier)) - 1, 0, 2);
    if (refresh != nullptr) {
        const int hz = config.refreshRate == RT64::UserConfiguration::RefreshRate::Original ? 20 :
            (config.refreshRate == RT64::UserConfiguration::RefreshRate::Display ? 60 : config.refreshRateTarget);
        *refresh = hz <= 20 ? 0 : (hz <= 30 ? 1 : 2);
    }
    if (filtering != nullptr) *filtering = std::clamp(static_cast<int>(config.filtering), 0, 2);
    if (downsample != nullptr) *downsample = config.downsampleMultiplier >= 2 ? 1 : 0;
#else
    if (scale != nullptr) *scale = 0;
    if (refresh != nullptr) *refresh = 2;
    if (filtering != nullptr) *filtering = 0;
    if (downsample != nullptr) *downsample = 0;
#endif
}
...
#ifdef WR64_SWITCH
    if (active_context == nullptr || active_context->application() == nullptr) return;
    auto &config = active_context->application()->userConfig;
    config.resolution = RT64::UserConfiguration::Resolution::Manual;
    config.resolutionMultiplier = static_cast<double>(std::clamp(scale, 0, 2) + 1);
    config.refreshRate = RT64::UserConfiguration::RefreshRate::Manual;
    config.refreshRateTarget = std::clamp(refresh, 0, 2) == 0 ? 20 :
        (std::clamp(refresh, 0, 2) == 1 ? 30 : 60);
    config.filtering = static_cast<RT64::UserConfiguration::Filtering>(std::clamp(filtering, 0, 2));
    config.downsampleMultiplier = std::clamp(downsample, 0, 1) + 1;
    config.validate();
    active_context->application()->updateUserConfig(true);
    active_context->application()->saveConfiguration();
    active_context->report_graphics_settings_public();
#else
    (void)scale; (void)refresh; (void)filtering; (void)downsample;
#endif
}
...
#ifdef WR64_SWITCH
    auto &state = wr64_switch_perf_state;
    snapshot->presentedFps = static_cast<float>(state.presentedFpsMilli.load(std::memory_order_relaxed)) / 1000.0f;
    snapshot->viFps = static_cast<float>(state.viFpsMilli.load(std::memory_order_relaxed)) / 1000.0f;
    snapshot->frameMs = static_cast<float>(state.frameMsMilli.load(std::memory_order_relaxed)) / 1000.0f;
    snapshot->presentMs = static_cast<float>(state.presentMsMilli.load(std::memory_order_relaxed)) / 1000.0f;
    snapshot->presented = state.snapshotPresented.load(std::memory_order_relaxed);
    snapshot->viUpdates = state.snapshotViUpdates.load(std::memory_order_relaxed);
    snapshot->presentAttempts = state.snapshotAttempts.load(std::memory_order_relaxed);
    snapshot->presentFailures = state.snapshotFailures.load(std::memory_order_relaxed);
    snapshot->width = state.width.load(std::memory_order_relaxed);
    snapshot->height = state.height.load(std::memory_order_relaxed);
    snapshot->nativeRate = state.nativeRate.load(std::memory_order_relaxed);
    snapshot->targetRate = state.targetRate.load(std::memory_order_relaxed);
    snapshot->swapChainRate = state.swapChainRate.load(std::memory_order_relaxed);
#else
    *snapshot = {};
#endif
}
